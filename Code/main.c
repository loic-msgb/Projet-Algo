...
void gestionEvenement(EvenementGfx evenement)
{
	...
	switch (evenement)
	{
		case Initialisation:
			...
			joueur.xPt1 = largeurFenetre()*0.5;
			joueur.xPt2 = largeurFenetre()*0.54;
			
			joueur.yPt1 = largeurFenetre()*0.5;
			joueur.yPt2 = largeurFenetre()*0.54;
		...
		case Temporisation:
			...
			if(compteur_temps == 0) stop = bloque_direction_bas(test, joueur, gravite);
			if(stop !=1){
				...
				if(condition_haut==1){
					while(compteur_temps <4){
						stop = bloque_direction_haut(test, joueur, saut);
						if(stop != 1){
			...
			if(condition_droite ==1){
				stop = bloque_direction_droite(test, joueur, vxJoueur);
				if(stop !=1){
			...
			if(condition_gauche ==1){
				stop = bloque_direction_gauche(test, joueur, vxJoueur);
				if(stop !=1){
					joueur = MoveLateral(joueur, -vxJoueur);
					condition_gauche = 0;
				}
